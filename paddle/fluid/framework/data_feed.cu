...
int GraphDataGenerator::FillSlotFeature(uint64_t *d_walk, size_t key_num) {
  ...
  if (fea_num == 0) {
    ...
    for (int i = 0; i < slot_num_; ++i) {
      ...
      if (feed_type_[i] == "uint64") {
        slot_tensor_ptr_[i] =
            (char*)(feed_vec_[3 + 2 * i]->mutable_data<int64_t>({1, 1}, this->place_));
        CUDA_CHECK(cudaMemsetAsync(
            slot_tensor_ptr_[i], 0, sizeof(int64_t), train_stream_));
      } else if (feed_type_[i] == "uint32") {
        slot_tensor_ptr_[i] =
            (char*)(feed_vec_[3 + 2 * i]->mutable_data<int32_t>({1, 1}, this->place_));
        CUDA_CHECK(cudaMemsetAsync(
            slot_tensor_ptr_[i], 0, sizeof(int32_t), train_stream_));
      } else if (feed_type_[i] == "float") {
        slot_tensor_ptr_[i] =
            (char*)(feed_vec_[3 + 2 * i]->mutable_data<float>({1, 1}, this->place_));
        CUDA_CHECK(cudaMemsetAsync(
  ...
  for (int i = 0; i < slot_num_; ++i) {
    // 判断slot类型
    if (feed_type_[i] == "uint64") {
      slot_tensor_ptr_[i] = (char*)(feed_vec_[3 + 2 * i]->mutable_data<int64_t>(
          {each_slot_fea_num[i], 1}, this->place_));
    } else if (feed_type_[i] == "uint32") {
      slot_tensor_ptr_[i] = (char*)(feed_vec_[3 + 2 * i]->mutable_data<int32_t>(
          {each_slot_fea_num[i], 1}, this->place_));
    } else if (feed_type_[i] == "float") {
      slot_tensor_ptr_[i] = (char*)(feed_vec_[3 + 2 * i]->mutable_data<float>(
          {each_slot_fea_num[i], 1}, this->place_));
    }
  }
