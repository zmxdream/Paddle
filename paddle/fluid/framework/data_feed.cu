...
__global__ void get_each_ins_float_info(uint8_t *slot_list,
                                        uint32_t *fea_size_list,
                                        uint32_t *fea_size_prefix,
                                        uint32_t *slot_size_list,
                                        uint32_t *slot_size_prefix,
                                        uint32_t *each_ins_slot_num,
                                        uint32_t *each_ins_slot_num_inner_prefix,
  ...
  if (i < key_num) {
    ...
    for (int j = 0; j < slot_size_list[i]; j++) {
      ...
      each_ins_slot_num[each_ins_slot_index + slot_list[slot_index + j]] += shape;
    }
    each_ins_slot_num_inner_prefix[each_ins_slot_index] = 0;
    for (int j = 1; j < slot_num; j++) {
      each_ins_slot_num_inner_prefix[each_ins_slot_index + j] =
...
int GraphDataGenerator::FillSlotFeature(uint64_t *d_walk, size_t key_num) {
  ...
  int fea_num =
      gpu_graph_ptr->get_feature_info_of_nodes(gpuid_,
                                               ...
                                               d_slot_list);
  // num of slot feature
  int slot_num = slot_num_ - float_slot_num_;
  ...
  if (fea_num == 0) {
    ...
    int ii = 0;
    for (int i = first_slot_idx_; i < slot_num_; ++i) { // slot_num_表示所有的slot包括float slot
      if (feed_type_[id_offset_of_feed_vec_ + 2 * i][0] == 'u') {
  ...
  std::shared_ptr<phi::Allocation> d_each_ins_slot_num_inner_prefix =
      memory::AllocShared(place_, (slot_num * key_num) * sizeof(uint32_t));
  ...
  int ii = 0;
  for (int i = first_slot_idx_; i < slot_num_; i++) {
    if (feed_type_[id_offset_of_feed_vec_ + 2 * i][0] == 'u') {
  ...
  if (first_slot_idx_ != -1) {
    ...
    // 如果第1个slot不是slot feature,是float feature咋办
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::InclusiveSum(NULL,
                                             temp_storage_bytes,
