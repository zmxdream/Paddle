...
/*
template <typename T>
void PrintLodTensorType(char* buf, Tensor* tensor, int64_t start, int64_t end) {
  auto count = tensor->numel();
  if (start < 0 || end > count) {
    VLOG(3) << "access violation";
    // return "access violation";
  ...
  int read_count = 0;
  for (int64_t i = start; i < end; i++) {
    *(T*)(buf + read_count) = tensor->data<T>()[i];
    read_count += sizeof(T);
    // os.append(":").append(std::to_string(tensor->data<T>()[i]));
  }
  // return os.str();
...
void PrintLodTensorIntType(char* buf, Tensor* tensor, int64_t start, int64_t end) {
  auto count = tensor->numel();
  if (start < 0 || end > count) {
    VLOG(3) << "access violation";
    // return "access violation";
  ...
  int read_count = 0;
  for (int64_t i = start; i < end; i++) {
    *(uint64_t*)(buf + read_count) = static_cast<uint64_t>(tensor->data<int64_t>()[i]);
    read_count += sizeof(uint64_t);
    //os.append(":").append(std::to_string(static_cast<uint64_t>(tensor->data<int64_t>()[i])));
  }
  //return os.str();
  //return os;
}

int GetLodTensorBytes(Tensor* tensor, int64_t start, int64_t end) {
  // std::string out_val;
  if (framework::TransToProtoVarType(tensor->dtype()) == proto::VarType::FP32) {
    // out_val = PrintLodTensorType<float>(tensor, start, end);
    return (end - start) * sizeof(float);
    ...
    return (end - start) * sizeof(double);
  } else {
    VLOG(3) << "unsupported type";
    return -1;
...
void PrintLodTensor(char* buf, Tensor* tensor, int64_t start, int64_t end) {
  // std::string out_val;
  if (framework::TransToProtoVarType(tensor->dtype()) == proto::VarType::FP32) {
    PrintLodTensorType<float>(buf, tensor, start, end);
  } else if (framework::TransToProtoVarType(tensor->dtype()) ==
             proto::VarType::INT64) {
    PrintLodTensorIntType(buf, tensor, start, end);
  } else if (framework::TransToProtoVarType(tensor->dtype()) ==
             proto::VarType::FP64) {
    PrintLodTensorType<double>(buf, tensor, start, end);
  } else {
    // out_val = "unsupported type";
  }
  // return out_val;
...
void DeviceWorker::DumpField(const Scope& scope, int dump_mode,
  ...
  std::vector<std::shared_ptr<MemRegion>> ars(batch_size, nullptr);
  ...
  auto fill_hit = [this, dump_interval, dump_mode, batch_size,
    ...
    for (size_t i = begin; i < end; i++) {
      ...
      ars[i] = std::make_shared<MemRegion>();
      int length = ins_id_vec[i].length();
      char* buf = ars[i]->acquire(length);

      // int read_count = 0;
      // copy insid
      memcpy(buf, ins_id_vec[i].data(), length);
      // read_count += length;
      ...
      for (auto& tensor : cpu_tensors) {
        ...
        int byte_num = GetLodTensorBytes(tensor.get(), bound.first, bound.second);       

        if (byte_num == -1) continue;
        buf = ars[i]->acquire(byte_num + 1); // 1 for '\t'
        ...
        //         std::to_string(bound.second - bound.first));
        //ars[i].append(PrintLodTensor(buf, tensor.get(), bound.first, bound.second));
        PrintLodTensor(buf, tensor.get(), bound.first, bound.second);
        // j++;
      }
      buf = ars[i]->acquire(1); // 1 for '\n'
      buf[0] = '\n';  
    }
