...
__global__ void FusedSeqpoolKernel(
    ...
    const float pad_value, const int cvm_offset, bool need_filter,
      ...
      if (need_filter) {
        ...
        for (int k = start; k < end; k++) {
          ...
          if (tid < cvm_offset) {  // show & click
            ...
            val += (static_cast<int>(
                       *(input_values[x] + k * embedding_size + tid) *
                           quant_ratio +
                       0.5)) /
        ...
        *(seqpool_output_values[x] + y * embedding_size + tid) = val;
        ...
        T val = static_cast<T>(0);
        // skip show click
        if (quant_ratio > 0 && tid >= cvm_offset) {
          // quant
          for (int k = start; k < end; k++) {
            val += (static_cast<int>(
                       *(input_values[x] + k * embedding_size + tid) *
...
void FusedSeqpoolCVMGrad(const paddle::platform::Place &place,
  ...
  DoFusedSeqpoolCVMGrad(place, gpu_out_grads_values,
                        ...
                        total_length, embedding_size, use_cvm, cvm_offset);
}
