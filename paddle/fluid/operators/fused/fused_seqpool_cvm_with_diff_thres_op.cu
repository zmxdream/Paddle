...
__global__ void FusedSeqpoolKernelQuantFilter(
    ...
    const bool xbox_diff_thres_filter, const float* threshold_vec_gpu) {
  CUDA_KERNEL_LOOP(i, N) {
    int key = i / embedding_size;
    ...
    for (auto k = start; k < end; ++k) {
      ...
      if (!xbox_diff_thres_filter) {
          ...
          if ((show - click) * show_coeff + click * clk_coeff < threshold_vec_gpu[x]) {
                continue;
          }
      }
      //if ((show - click) * show_coeff + click * clk_coeff < threshold) {
...
void FusedSeqpoolCVM(const paddle::platform::Place &place,
  ...
  cudaMemcpyAsync(threshold_vec_gpu, threshold_vec.data(), threshold_vec.size() * sizeof(float),
                  cudaMemcpyHostToDevice, stream);

  //float* threshold_vec_gpu = reinterpret_cast<float*>(lods_values[lods.size()]);
  //cudaMemcpyAsync(threshold_vec_gpu, threshold_vec.data(), threshold_vec.size() * sizeof(float),
  //                cudaMemcpyHostToDevice, stream);
  //VLOG(0) << "succ copy threshold_vec.";
  ...
  if (need_filter) {  // quant need filter
        FusedSeqpoolKernelQuantFilter<<<GET_BLOCK(N), PADDLE_CUDA_NUM_THREADS, 0,
            ...
            xbox_diff_thres_filter, threshold_vec_gpu);
...
template <typename T>
class FusedSeqpoolCVMWithDiffThresGradCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
