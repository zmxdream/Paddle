...
class FusedSeqpoolCVMTradeWOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    const int trade_num = ctx->Attrs().Get<int>("trade_num");
    ...
    for (size_t i = 0; i < num_inputs; ++i) {
      ...
      if (use_cvm) {
        out_dim = {-1, dims[rank - 1] - trade_num};
      } else {
...
class FusedSeqpoolCVMTradeWOpMaker : public framework::OpProtoAndCheckerMaker {
 public:
  void Make() override {
    AddInput("X",
    ...
    AddAttr<int>("trade_id", "(int, default -1)").SetDefault(-1);
    AddAttr<int>("trade_num", "(int, default 2)").SetDefault(2);
...
class FusedSeqpoolCVMTradeWGradOp : public framework::OperatorWithKernel {
 public:
  using framework::OperatorWithKernel::OperatorWithKernel;
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    for (size_t i = 0; i < og_dims.size(); i++) {
      ...
      if (use_cvm) {
        ...
        PADDLE_ENFORCE_EQ(
            o_dim + trade_num, x_dims[i][og_dims[i].size() - 1],
            platform::errors::InvalidArgument(
        ...
        PADDLE_ENFORCE_EQ(
            ...
            x_dims[i][og_dims[i].size() - 1] - cvm_offset - trade_num,
            platform::errors::InvalidArgument(
                "The dimension mismatch between Input(OUT@GRAD) and "
                "Input(X). Received Input(OUT@GRAD): input rank %u, "
                "input shape [%s]; received Input(X): input rank %u, "
...
template <typename T>
class FusedSeqpoolCVMTradeWGradOpMaker
 ...
 protected:
  void Apply(GradOpPtr<T> op_desc_ptr) const override {
    op_desc_ptr->SetType("fused_seqpool_cvm_tradew_grad");
...
REGISTER_OPERATOR(
    fused_seqpool_cvm_tradew, ops::FusedSeqpoolCVMTradeWOp,
    ops::FusedSeqpoolCVMTradeWOpMaker,
    ops::FusedSeqpoolCVMTradeWGradOpMaker<paddle::framework::OpDesc>,
    ops::FusedSeqpoolCVMTradeWGradOpMaker<paddle::imperative::OpBase>);
REGISTER_OPERATOR(fused_seqpool_cvm_tradew_grad,
                  ops::FusedSeqpoolCVMTradeWGradOp)

REGISTER_OP_CPU_KERNEL(fused_seqpool_cvm_tradew,
                       ops::FusedSeqpoolCVMTradeWOpCPUKernel<float>)
REGISTER_OP_CPU_KERNEL(fused_seqpool_cvm_tradew_grad,
                       ops::FusedSeqpoolCVMTradeWGradOpCPUKernel<float>)
