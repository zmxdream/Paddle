...
template <typename T>
__global__ void KernelMaskedDataNormFF(
    int N, int C, const T *x, const bool *mask, T *y
...
template <typename T>
__global__ void KernelMaskedDataNormBP(
    int N, int C, const bool *mask, const T *y_grad
    , const T *scale, T *x_grad) {
  CUDA_KERNEL_LOOP(i, N * C) {
    int row = i / C;
    if (mask[row]) {
      x_grad[i] = y_grad[i] * scale[i % C];
    } else {
      x_grad[i] = 0;
    }
  }
...
template <typename T>
__global__ void KernelMaskedDataNormBPStat(int N,
                                     int C,
                                     const T *x_val,
                                     const bool *mask,
                                     const T *means,
  ...
  CUDA_KERNEL_LOOP(i, C) {
    ...
    if (mask_num > 0) {
      batch_size[i] = 1;
      batch_sum[i] = val_sum / mask_num;
      batch_square_sum[i] = square_sum / mask_num + squared_sum_epsilon;
    } else {
      batch_size[i] = 0;
      batch_sum[i] = 0;
      batch_square_sum[i] = 0;
    }
  }
}
...
__global__ void KernelUpdateParam(int C,
  ...
  CUDA_KERNEL_LOOP(i, C) {
    if (d_batch_size[i] > 0) {  // avoid empty decay
...
template <typename T>
class MaskedDataNormKernel<phi::GPUContext, T> : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    const auto *x = ctx.Input<Tensor>("X");
    const auto *mask = ctx.Input<Tensor>("Mask");
    ...
    PADDLE_ENFORCE_EQ(
        ...
        platform::errors::InvalidArgument(
            "Mask's numbers must be equal to X's N, "
            "when X's N is %d. But Mask's numbers %d",
            N, mask->numel()));
    ...
    KernelMeanScale<<<GET_BLOCKS(C), PADDLE_CUDA_NUM_THREADS, 0, stream>>>(
        ...
        mean_out_data,
        scale_out_data);
    KernelMaskedDataNormFF<<<GET_BLOCKS(C * N)
                             , PADDLE_CUDA_NUM_THREADS, 0, stream>>>(
        N, C, x_data, mask_data, y_data, mean_out_data, scale_out_data);
  }
};

template <typename T>
class MaskedDataNormGradKernel<phi::GPUContext, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    auto stream = ctx.template device_context<phi::GPUContext>().stream();
    if (d_x != nullptr) {
      KernelMaskedDataNormBP<<<GET_BLOCKS(C * N),
    ...
    KernelMaskedDataNormBPStat<<<GET_BLOCKS(C)
        , PADDLE_CUDA_NUM_THREADS, 0, stream>>>(
            N,
            C,
            x->data<T>(),
...
REGISTER_OP_CUDA_KERNEL(masked_data_norm,
                        ops::MaskedDataNormKernel<phi::GPUContext, float>,
                        ops::MaskedDataNormKernel<phi::GPUContext, double>);
REGISTER_OP_CUDA_KERNEL(masked_data_norm_grad,
                        ops::MaskedDataNormGradKernel<phi::GPUContext, float>,
                        ops::MaskedDataNormGradKernel<phi::GPUContext, double>);
