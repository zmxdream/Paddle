...
PADDLE_API std::vector<std::vector<Tensor>> matmul_grad(const Tensor& x, const Tensor& y, const Tensor& out_grad, bool transpose_x, bool transpose_y) {
  ...
  pten::MatmulGradInferMeta(MakeMetaTensor(*input_x), MakeMetaTensor(*input_y), MakeMetaTensor(*input_out_grad), transpose_x, transpose_y, &meta_out_0, &meta_out_1);


  using kernel_signature = void(*)(const platform::DeviceContext&, const pten::DenseTensor&, const pten::DenseTensor&, const pten::DenseTensor&, bool, bool, pten::DenseTensor*, pten::DenseTensor*);
...
Tensor scale_grad(const Tensor& out_grad, const Scalar& scale_val, float bias, bool bias_after_scale) {
