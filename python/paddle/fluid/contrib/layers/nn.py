...
def fused_seqpool_cvm_tradew(input,
                             pool_type,
                             cvm,
                             pad_value=0.0,
                             use_cvm=True,
    ...
    Returns:
        ...
        of input.
    """
    helper = LayerHelper('fused_seqpool_cvm_tradew', **locals())
    ...
    check_type(input, 'input', list, 'fused_seqpool_cvm_tradew')
    if isinstance(input, list):
        for _input in input:
            check_variable_and_dtype(_input, 'input', ['float32'],
                                     'fused_seqpool_cvm_tradew')
...
def fused_seqpool_cvm_with_credit(input,
                                pool_type,
                                cvm,
                                pad_value=0.0,
                                use_cvm=True,
    ...
    Returns:
        ...
        of input.
    """
    helper = LayerHelper('fused_seqpool_cvm_with_credit', **locals())

    if pool_type.upper() != 'SUM':
        raise ValueError(
            "fused_seqpool_cvm_with_credit only support SUM pooling now, and your type is: "
            + pool_type)

    check_type(input, 'input', list, 'fused_seqpool_cvm_with_credit')
    if isinstance(input, list):
        for _input in input:
            check_variable_and_dtype(_input, 'input', ['float32'],
                                     'fused_seqpool_cvm_with_credit')
    ...
    helper.append_op(
        type="fused_seqpool_cvm_with_credit",
        inputs={"X": inputs,
                "CVM": cvm},
        outputs={"Out": outs},
        attrs={
