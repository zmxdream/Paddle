...
__all__ = [
    ...
    'HeterXpuTrainer', 'HeterPipelineTrainer', 'BoxPSTrainer'
]
...
class BoxPSTrainer(TrainerDesc):
    """
    Implement of BoxPSTrainer.
    It's for Pipeline.
    """

    def __init__(self):
        super(BoxPSTrainer, self).__init__()
        pass

    def _set_program(self, program):
        super(BoxPSTrainer, self)._set_program(program)
        self._program = program

    def _gen_trainer_desc(self):
        super(BoxPSTrainer, self)._gen_trainer_desc()
        self.proto_desc.class_name = "BoxPSTrainer"
        if self._program == None:
            raise RuntimeError("None Program")
        self._device_worker._set_infer(self._infer)
        self._device_worker._set_program(self._program)
